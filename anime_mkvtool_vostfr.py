...
def find_mkvmerge() -> Optional[str]:
    ...
    """
    # TODO: Ajouter la détection pour Linux/MacOS
    default_paths = [
...
def get_mkv_tracks(input_path, mkvmerge_path):
    ...
    try:
        ...
        print(f"Erreur lors de la récupération des informations des pistes : {e}")
        return None
...
def detect_gpu():
    ...
    """
    try:
        # Commande pour détecter les GPU
...
def calculate_optimal_processes():
    ...
    estimated_memory_per_process = 1024 * 1024 * 500  # 500 Mo par processus
    ...
    optimal_processes = min(
        ...
        max(total_cores // 2, 4)  # Au moins 4 processus, ou la moitié des cœurs
    )
...
def process_single_mkv(mkvmerge_path, input_dir, output_dir, filename, selected_main_subtitle, selected_subtitles):
    ...
    if not track_info:
        print(f"Impossible de traiter {filename}")
        return False
    ...
    if japanese_audio is None:
        print(f"Piste audio japonaise non trouvée dans {filename}")
        return False
    ...
    if not matching_main_subtitle:
        print(f"La piste de sous-titres principale n'est pas trouvée dans {filename}")
        return False
    ...
    try:
        ...
        print(f"Pistes de sous-titres dans {filename}:")
        for subtitle_id in subtitle_tracks_to_keep:
            subtitle = next((s for s in french_subtitles if s['id'] == subtitle_id), None)
            if subtitle:
                print(f"  - Piste {subtitle['id']}: {subtitle.get('properties', {}).get('track_name', 'N/A')}")
        
        # Information sur les pistes supprimées
        if french_audio_tracks:
            print(f"Pistes audio françaises supprimées dans {filename}: {', '.join(french_audio_tracks)}")
...
def process_mkv_files(input_dir, output_dir, mkvmerge_path):
    ...
    # Détecter le GPU
    gpu_type = detect_gpu()
    print(f"GPU détecté : {gpu_type if gpu_type else 'Aucun GPU spécifique'}")
    ...
    print(f"Nombre de processus utilisés : {num_processes}")
    print("\n--- Analyse de l'espace disque ---")
    print(f"Taille initiale : {format_size(initial_size)}")
    print(f"Taille finale : {format_size(final_size)}")
    print(f"Espace économisé : {format_size(saved_size)} ({saved_percentage:.2f}%)")
